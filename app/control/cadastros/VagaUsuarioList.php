...
class VagaUsuarioList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_VagaUsuario';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle("Listagem de Candidatos por vaga");
        ...
        $btn_onshow = $this->form->addAction("Novo Vínculo", new TAction(['VagaUsuarioForm', 'onShow']), 'fas:plus #69aa46');
        ...
        $this->datagrid->addColumn($column_vaga_titulo);

        $action_onEdit = new TDataGridAction(array('VagaUsuarioForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('VagaUsuarioList', 'onDelete'));
        ...
        $container->add(TBreadCrumb::create(["Vagas","Candidato por Vaga"]));
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new VagaUsuario($key, FALSE); 
    ...
    public function onSearch()
    {
        ...
        if (isset($data->vaga_id) AND ( (is_scalar($data->vaga_id) AND $data->vaga_id !== '') OR (is_array($data->vaga_id) AND (!empty($data->vaga_id)) )) )
        {

            $filters[] = new TFilter('vaga_id', '=', $data->vaga_id);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for VagaUsuario
