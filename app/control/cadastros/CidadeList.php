...
class CidadeList extends BasePage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $row1 = $this->form->addFields([new TLabel("UF:", null, '14px', null)],[$uf_id],[new TLabel("Código IBGE:", null, '14px', null)],[$codigo_ibge]);
        ...
        $column_uf_nome = new TDataGridColumn('uf->nome', "UF", 'left');
        $column_codigo_ibge = new TDataGridColumn('codigo_ibge', "Código IBGE", 'left');

        $this->datagrid->addColumn($column_nome);
        $this->datagrid->addColumn($column_uf_nome);
    ...
    public function onSearch()
    {
        ...
        if (isset($data->codigo_ibge) AND ( (is_scalar($data->codigo_ibge) AND $data->codigo_ibge !== '') OR (is_array($data->codigo_ibge) AND (!empty($data->codigo_ibge)) )) )
        {
            ...
            $filters[] = new TFilter('codigo_ibge', '=', $data->codigo_ibge);// create the filter 
        }
