...
$input.val();var key=difference($.oldValue,$.newValue).charCodeAt(0);keypressEvent(e,key)});$input.on("keyup",function(e){var selection=getInputSelection();var startPos=selection.start;maskAndPosition(startPos+1)})}}function keypressEvent(e,keyCode){e=e||window.event;var key=keyCode||e.which||e.charCode||e.keyCode,decimalKeyCode=settings.decimal.charCodeAt(0);if(key===undefined)return false;if(key===229)return false;if((key<48||key>57)&&(key!==decimalKeyCode||!settings.reverse))return handleAllKeysExceptNumericalDigits(key,
e);else if(!canInputMoreNumbers())return false;else{if(key===decimalKeyCode&&shouldPreventDecimalKey())return false;if(settings.formatOnBlur)return true;preventDefault(e);applyMask(e);return false}}function shouldPreventDecimalKey(){if(isAllTextSelected())return false;return alreadyContainsDecimal()}function isAllTextSelected(){var length=$input.val().length;var selection=getInputSelection();return selection.start===0&&selection.end===length}function alreadyContainsDecimal(){return $input.val().indexOf(settings.decimal)>
-1}function applyMask(e){e=e||window.event;var key=e.which||e.charCode||e.keyCode,keyPressedChar="",selection,startPos,endPos,value;if(key==229)key=$input.val().charCodeAt($input.val().length-1);if(key>=48&&key<=57)keyPressedChar=String.fromCharCode(key);selection=getInputSelection();startPos=selection.start;endPos=selection.end;value=$input.val();$input.val(value.substring(0,startPos)+keyPressedChar+value.substring(endPos,value.length));maskAndPosition(startPos+1)}function handleAllKeysExceptNumericalDigits(key,
e){if(key===45){$input.val(changeSign());return false}else if(key===43){$input.val($input.val().replace("-",""));return false}else if(key===13||key===9)return true;else if($.browser.mozilla&&(key===37||key===39)&&e.charCode===0)return true;else{preventDefault(e);return true}}function keydownEvent(e){e=e||window.event;var key=e.which||e.charCode||e.keyCode,selection,startPos,endPos,value,lastNumber;if(key===undefined)return false;if(key===229)key=$input.val().charCodeAt($input.val().length-1);selection=
getInputSelection();startPos=selection.start;endPos=selection.end;if(key===8||key===46||key===63272){preventDefault(e);value=$input.val();if(startPos===endPos)if(key===8)if(settings.suffix==="")startPos-=1;else{lastNumber=value.split("").reverse().join("").search(/\d/);startPos=value.length-lastNumber-1;endPos=startPos+1}else endPos+=1;$input.val(value.substring(0,startPos)+value.substring(endPos,value.length));maskAndPosition(startPos);return false}else if(key===9)return true;else return true}function focusEvent(){onFocusValue=
$input.val();mask();var input=$input.get(0),textRange;if(!!settings.selectAllOnFocus)input.select();else if(input.createTextRange&&settings.bringCaretAtEndOnFocus){textRange=input.createTextRange();textRange.collapse(false);textRange.select()}}function cutPasteEvent(){setTimeout(function(){mask()},0)}function getDefaultMask(){var n=parseFloat("0")/Math.pow(10,settings.precision);return n.toFixed(settings.precision).replace(new RegExp("\\.","g"),settings.decimal)}function blurEvent(e){if($.browser.msie)keypressEvent(e,
null);if(!!settings.formatOnBlur&&$input.val()!==onFocusValue)applyMask(e);if($input.val()===""&&settings.allowEmpty)$input.val("");else if($input.val()===""||$input.val()===setSymbol(getDefaultMask(),settings))if(!settings.allowZero)$input.val("");else if(!settings.affixesStay)$input.val(getDefaultMask());else $input.val(setSymbol(getDefaultMask(),settings));else if(!settings.affixesStay){var newValue=$input.val().replace(settings.prefix,"").replace(settings.suffix,"");$input.val(newValue)}if($input.val()!==
onFocusValue)$input.change()}function clickEvent(){var input=$input.get(0),length;if(!!settings.selectAllOnFocus)return;else if(input.setSelectionRange&&settings.bringCaretAtEndOnFocus){length=$input.val().length;input.setSelectionRange(length,length)}else $input.val($input.val())}function doubleClickEvent(){var input=$input.get(0),start,length;if(input.setSelectionRange&&settings.bringCaretAtEndOnFocus){length=$input.val().length;start=settings.doubleClickSelection?0:length;input.setSelectionRange(start,
length)}else $input.val($input.val())}fixMobile();$input.unbind(".maskMoney");$input.bind("keypress.maskMoney",keypressEvent);$input.bind("keydown.maskMoney",keydownEvent);$input.bind("blur.maskMoney",blurEvent);$input.bind("focus.maskMoney",focusEvent);$input.bind("click.maskMoney",clickEvent);$input.bind("dblclick.maskMoney",doubleClickEvent);$input.bind("cut.maskMoney",cutPasteEvent);$input.bind("paste.maskMoney",cutPasteEvent);$input.bind("mask.maskMoney",mask)})}};function setSymbol(value,settings){var operator=
"";if(value.indexOf("-")>-1){value=value.replace("-","");operator="-"}if(value.indexOf(settings.prefix)>-1)value=value.replace(settings.prefix,"");if(value.indexOf(settings.suffix)>-1)value=value.replace(settings.suffix,"");return operator+settings.prefix+value+settings.suffix}function maskValue(value,settings){if(settings.allowEmpty&&value==="")return"";if(!!settings.reverse)return maskValueReverse(value,settings);return maskValueStandard(value,settings)}function maskValueStandard(value,settings){var negative=
value.indexOf("-")>-1&&settings.allowNegative?"-":"",onlyNumbers=value.replace(/[^0-9]/g,""),integerPart=onlyNumbers.slice(0,onlyNumbers.length-settings.precision),newValue,decimalPart,leadingZeros;newValue=buildIntegerPart(integerPart,negative,settings);if(settings.precision>0){decimalPart=onlyNumbers.slice(onlyNumbers.length-settings.precision);leadingZeros=(new Array(settings.precision+1-decimalPart.length)).join(0);newValue+=settings.decimal+leadingZeros+decimalPart}return setSymbol(newValue,
settings)}function maskValueReverse(value,settings){var negative=value.indexOf("-")>-1&&settings.allowNegative?"-":"",valueWithoutSymbol=value.replace(settings.prefix,"").replace(settings.suffix,""),integerPart=valueWithoutSymbol.split(settings.decimal)[0],newValue,decimalPart="";if(integerPart==="")integerPart="0";newValue=buildIntegerPart(integerPart,negative,settings);if(settings.precision>0){var arr=valueWithoutSymbol.split(settings.decimal);if(arr.length>1)decimalPart=arr[1];newValue+=settings.decimal+
decimalPart;var rounded=Number.parseFloat(integerPart+"."+decimalPart).toFixed(settings.precision);var roundedDecimalPart=rounded.toString().split(".")[0];newValue=newValue.split(settings.decimal)[0]+"."+roundedDecimalPart}return setSymbol(newValue,settings)}function buildIntegerPart(integerPart,negative,settings){integerPart=integerPart.replace(/^0*/g,"");integerPart=integerPart.replace(/\B(?=(\d{3})+(?!\d))/g,settings.thousands);if(integerPart==="")integerPart="0";return negative+integerPart}$.fn.maskMoney=
function(method){if(methods[method])return methods[method].apply(this,Array.prototype.slice.call(arguments,1));else if(typeof method==="object"||!method)return methods.init.apply(this,arguments);else $.error("Method "+method+" does not exist on jQuery.maskMoney")}})(window.jQuery||window.Zepto);
